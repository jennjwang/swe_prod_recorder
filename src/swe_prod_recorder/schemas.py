...
class AuditSchema(BaseModel):
    ...
    is_new_information: bool = Field(
        ..., description="Whether the message reveals anything not seen before"
    )
    ...
    transmit_data: bool = Field(
        ..., description="Should downstream processing continue"
    )
...
class RelationItem(BaseModel):
    ...
    model_config = ConfigDict(
        extra="forbid", json_schema_extra={"required": ["source", "label", "target"]}
    )
...
class RelationSchema(BaseModel):
    ...
    model_config = ConfigDict(
        extra="forbid", json_schema_extra={"required": ["relations"]}
    )
